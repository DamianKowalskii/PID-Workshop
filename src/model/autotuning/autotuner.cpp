...
static double convert_genome_bits_to_double(uint8_t *genome,
                                            ...
                                            int32_t index_stop,
                                            uint32_t chunk_size) {
  boost::dynamic_bitset<> dbs(chunk_size, 0);
  dbs.reset();
  uint32_t dbs_iterator = 0;
  for (int i = index_start; i <= index_stop; ++i, ++dbs_iterator) {
    if (genome[i] == 1) {
      dbs[dbs_iterator] = 1;
    }
  }
  ...
  int16_t value = static_cast<int16_t>(dbs.to_ulong());
...
static void rate_pid(pid_rating_t *pid_rating, std::vector<double> x,
  ...
  std::vector<double>::iterator it = std::max_element(y.begin(), y.end());
    ...
    */
    ...
    if (is_signal_unstable_after_setpoint) {
      ...
      */
      ...
      auto times = nc::NdArray<double>(x);  // timestamp after reaching setpoint
      auto signal = nc::NdArray<double>(y);
      double times_mean = nc::mean(times)[0];
      double signal_mean = nc::mean(signal)[0];
      auto times_diffrence = times - times_mean;
      auto signal_diffrence = signal - signal_mean;
      auto product = times_diffrence * signal_diffrence;
      auto numerator = nc::sum(product);
      auto denominator = nc::sum(nc::power(times_diffrence, 2));
      auto b1 = (numerator / denominator)[0];
      auto b0 = signal_mean - (b1 * times_mean);
      std::tuple<double, double> coefficients(b0, b1);
      auto predicted_signal = nc::empty<double>(1, times.size());
      int32_t times_iterator = 0;
      for (auto &element : predicted_signal) {
        element = predict(times[times_iterator], coefficients);
        ++times_iterator;
      }
      auto errors = signal - predicted_signal;
      auto squared_summed_errors = nc::sum(nc::power(errors, 2))[0];
      auto reverse_errors = nc::flip(errors, nc::Axis::ROW);
      double sum_of_diffrence = 0;
      for (auto it = reverse_errors.begin(); it < reverse_errors.end(); ++it) {
        sum_of_diffrence += (*it) - *(it + 1);
      }
      /* The rule of thumb is that a value between 1.5-2.5 indicates,
         that there is no autocorrelation in the signal.
      */
      durpin_watson = sum_of_diffrence / squared_summed_errors;
